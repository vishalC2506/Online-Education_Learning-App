...
const ResourceForm = ({ section, courseId }: ResourceFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      console.log("Failed to Upload the resource", err);
    }
  };
  ...
  return (
    <>
      ...
      <p className="text-sm font-medium mt-2">
        Add resources to this section to learn students to learn better{" "}
      </p>
      <div className="mt-5 flex flex-col gap-5">
        {section.resources.map((resource) => (
          <div
            key={resource.id}
            className="flex justify-between bg-[#FFF8EB] rounded-lg text-sm
                       font-medium p-3"
          >
            ...
            <button className="text-[#FDAB04]" disabled={isSubmitting}>
              {isSubmitting ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
                <X className="h-4 w-4" onClick={() => onDelete(resource.id)} />
              )}
            </button>
          </div>
        ))}
